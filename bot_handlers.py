...
# Importa todas as funções do bot (estas devem estar no mesmo arquivo ou importadas corretamente)
...
# Defina aqui os estados da conversação
NOME, TELEFONE, PACOTE, VALOR, SERVIDOR, VENCIMENTO, CONFIRMAR = range(7)

# ConversationHandler para cadastro de clientes
cadastro_handler = ConversationHandler(
    ...
    fallbacks=[MessageHandler(filters.Regex("^❌ Cancelar$"), cancelar_cadastro)],
    allow_reentry=True,
)
...
async def main():
    ...
    application.add_handler(CallbackQueryHandler(callback_cliente))
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, lidar_com_botoes))
